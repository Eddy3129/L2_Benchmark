...
interface GroupedEstimationRecord {
  timestamp: string;
  records: GasEstimationRecord[];
  networkCount: number;
  contractName: string;
}

export default function GasEstimationTable() {
  const [records, setRecords] = useState<GasEstimationRecord[]>([]);
  const [groupedRecords, setGroupedRecords] = useState<GroupedEstimationRecord[]>([]);
  ...
  const groupRecordsByTimestamp = (records: GasEstimationRecord[]): GroupedEstimationRecord[] => {
    const grouped = records.reduce((acc, record) => {
      ...
      return acc;
    }, {} as Record<string, GasEstimationRecord[]>);
  ...
  const exportTimestampToCSV = (groupedRecord: GroupedEstimationRecord) => {
    const csvContent = [
      'Network,Contract,Measured Gas Used,L2 Gas Price (gwei),Token Price (USD),Est. Deployment Cost (USD),Est. L2 Execution (USD),Est. L1 Blob Cost (USD),vs. Ethereum,Confidence Level',
      ...groupedRecord.records.map(record => 
        `${record.network},${record.contractName},${record.measuredGasUsed},${record.l2GasPriceGwei || '—'},${record.tokenPriceUsd},${record.estDeploymentCostUsd},${record.estL2ExecutionUsd || '—'},${record.estL1BlobCostUsd || '—'},${record.vsEthereum},${record.confidenceLevel}%`
      )
    ].join('\n');
    ...
    link.setAttribute('download', `gas_estimation_${groupedRecord.contractName}_${new Date(groupedRecord.timestamp).toISOString().replace(/[:.]/g, '-')}.csv`);
  ...
  const fetchRecords = async () => {
    try {
      ...
      const response = await fetch(`${backendUrl}/api/gas-estimation/records?${params}`);
      if (!response.ok) {
      ...
      console.error('Failed to fetch gas estimation records:', err);
  ...
  const fetchAllRecords = async () => {
    try {
      ...
      const response = await fetch(`${backendUrl}/api/gas-estimation/records?limit=100&sortBy=timestamp&sortOrder=desc`);
      if (!response.ok) {
      ...
      console.error('Failed to fetch all gas estimation records:', err);
  ...
  const deleteRecordsByTimestamp = async (timestamp: string) => {
    if (!confirm(`Are you sure you want to delete all estimation records for timestamp ${formatTimestamp(timestamp)}?`)) {
      return;
    }
    
    setIsDeleting(true);
    try {
      const response = await fetch(`${backendUrl}/api/gas-estimation/records/timestamp/${encodeURIComponent(timestamp)}`, {
          method: 'DELETE'
        });
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="flex items-center gap-3 text-gray-400">
          ...
          <span>Loading gas estimation data...</span>
        </div>
      </div>
    );
  }
        ...
        <CardContent className="pt-0">
          {(viewMode === 'grouped' ? groupedRecords.length === 0 : records.length === 0) ? (
            <div className="text-center py-8 text-gray-400">
              ...
              <p>No gas estimation data found</p>
              <p className="text-sm mt-1">Try adjusting your filters or run some gas estimations</p>
            </div>
          ) : (
                  ...
                  <TableRow className="border-gray-700">
                    ...
                    {viewMode === 'grouped' ? (
                      ...
                      <>
                        ...
                        <TableHead className="text-gray-300">Contract</TableHead>
                        <TableHead className="text-gray-300 text-right">Gas Used</TableHead>
                        <TableHead className="text-gray-300 text-right">L2 Gas Price</TableHead>
                        <TableHead className="text-gray-300 text-right">Token Price</TableHead>
                        <TableHead className="text-gray-300 text-right">Deployment Cost</TableHead>
                        <TableHead className="text-gray-300 text-right">vs Ethereum</TableHead>
                  ...
                  {viewMode === 'grouped' ? (
                                ...
                                <Table>
                                  <TableHeader>
                                    <TableRow className="border-gray-600">
                                      ...
                                      <TableHead className="text-gray-400 text-sm text-right">Gas Used</TableHead>
                                      <TableHead className="text-gray-400 text-sm text-right">L2 Gas Price</TableHead>
                                      <TableHead className="text-gray-400 text-sm text-right">Token Price</TableHead>
                                      <TableHead className="text-gray-400 text-sm text-right">Deployment Cost</TableHead>
                                      <TableHead className="text-gray-400 text-sm text-right">L2 Execution</TableHead>
                                      <TableHead className="text-gray-400 text-sm text-right">L1 Blob Cost</TableHead>
                                      <TableHead className="text-gray-400 text-sm text-right">vs Ethereum</TableHead>
                                      <TableHead className="text-gray-400 text-sm text-right">Confidence</TableHead>
                                    </TableRow>
                                  </TableHeader>
                                  <TableBody>
                                    {groupedRecord.records.map((record) => (
                                      <TableRow key={record.id} className="border-gray-600 hover:bg-gray-800/30">
                                        ...
                                        <TableCell className="text-sm text-right text-gray-300 font-mono">
                                          {parseInt(record.measuredGasUsed).toLocaleString()}
                                        </TableCell>
                                        <TableCell className="text-sm text-right text-gray-300 font-mono">
                                          {formatGwei(record.l2GasPriceGwei)}
                                        </TableCell>
                                        <TableCell className="text-sm text-right text-gray-300 font-mono">
                                          {formatCurrency(record.tokenPriceUsd)}
                                        </TableCell>
                                        <TableCell className="text-sm text-right text-white font-mono">
                                          {formatCurrency(record.estDeploymentCostUsd)}
                                        </TableCell>
                                        <TableCell className="text-sm text-right text-yellow-300 font-mono">
                                          {record.estL2ExecutionUsd ? formatCurrency(record.estL2ExecutionUsd) : '—'}
                                        </TableCell>
                                        <TableCell className="text-sm text-right text-blue-300 font-mono">
                                          {record.estL1BlobCostUsd ? formatCurrency(record.estL1BlobCostUsd) : '—'}
                                        </TableCell>
                    ...
                    records.map((record) => (
                      <TableRow key={record.id} className="border-gray-700 hover:bg-gray-700/30">
                        ...
                        <TableCell className="text-right text-green-300 font-mono">
                          {record.vsEthereum}
                        </TableCell>
                        <TableCell className="text-right text-gray-300 font-mono">
                          {record.confidenceLevel}%
